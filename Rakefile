...
PACKAGE_NAME = "stripeiif"
VERSION = "1.0.0"
TRAVELING_RUBY_VERSION = "20150210-2.1.5"

desc "Package stripeiif"
...
def create_package(target, os_type = :unix)
  package_dir = "package/#{PACKAGE_NAME}-#{VERSION}-#{target}"
  ...
  if os_type == :unix
    sh "cp packaging/wrapper.sh #{package_dir}/stripeiif"
  else
    sh "cp packaging/wrapper.bat #{package_dir}/stripeiif.bat"
  end
